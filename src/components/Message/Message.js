...
import Media from "../MediaRenders/Media";
...
import { 
    chatsActions, groupChatsActions, 
    messageActions, statusMessageActions,
    selectedChatsActions 
} from "../../state/index";
import useLongPress from "../../hooks/useLongPress";
import useSwipe from "../../hooks/useSwipe";
import { formatTextForLinks, formatToTimeAndDate, formatMessageTime } from "../../utils/formatters";
import StatusTagged from "../StatusTagged/StatusTagged";
import ReactionEmojiPicker from "../reaction-emoji-picker";
import EmojiPicker from "emoji-picker-react";
import useClickOutside from "../../hooks/useClickOutside";
import { fetchFakeReactions } from "../../utils/routings";

const Message = ({ 
    idx, data, selectedChats, setSelectedChats, 
    showTop, group, setClickAccount, id, socket 
}) => {
    ...
    const contactSaved = (userId) => {
        // console.log(contacts, userId, data);
    ...
    const getAllReactions = () => {
        ...
        setAllReactions(fetchFakeReactions());
        return setReactionsLoading(false);

        setReactionsLoading(true);
        getReactions(data._id, group ? "gc" : "dm")
            .then(res => {
    ...
    const sendReaction = (emoji) => {
        postReaction(data._id, group ? "gc" : "dm", emoji)
            .then(res => {
                updateReaction(null, null, group ? groups : chats, group ? setGroupChatsData : setChatsData, data.id, data.messageId, res.data.reactions);
                socket.emit('postReaction', {
                    reactions: res.data.reactions, id: data._id,
                    sender: _id, groupId: (group ? id : null), receiver: (group ? null : id),
                });
    ...
    const handleRemoveReaction = (val) => {
        //if we are not d one dat posted this reaction we cannot delete it
        if(val.userId !== _id || removeReactionsLoading) return;
        setRemoveReactionsLoading(true);
        removeReaction(data._id, group ? "gc" : "dm")
            .then(res => {
                ...
                socket.emit('removeReaction', {
                    ...
                    deleter: _id, groupId: (group ? id : null), receiver: (group ? null : id),
                });
                updateReaction(null, null, group ? groups : chats, group ? setGroupChatsData : setChatsData, data.id, data.messageId, res.data.reactions);
            })
            .catch(err => {
    ...
    return (
        <div className={`Message ${data.senderId==_id?"you":"others"}`} {...clickEvents}>
            <div className="Message__Wrapper" {...swipeEvents}
            style={{transform: (ele===data.messageId && pos) && `translateX(${pos}px)`}}>
                {showTop && <div className="Message__Top"
                onClick={() => {
                    if(!group) return;
                    if(!setClickAccount) return;
                    setClickAccount({number: data.senderNumber, userId: data.senderId, clicked: true})
                }}>
                    <div className="senderName" style={{color: data.senderColor||"black"}}>
                        {/* {console.log(data.senderId)} */}
                        {data.senderId == _id ? 'You' : contactSaved(data.senderId)?.userName||`- ${data.senderName}`}
                    </div>
                    {!contactSaved(data.userId) && 
                ...
                {data.messageTagged && 
                    <TaggedText 
                    input={false}
                    taggedData={data.messageTagged}
                    contactSaved={contactSaved(data.senderId)}
                    />
                }
                {data.status && <StatusTagged status={data.status} />}
                {data?.images?.length > 0 && <div className="Message__Media">
                    <Media mediaData={data.images}
                    senderName={data.senderId==_id ? "You": (contactSaved(data.senderId)?.userName||data.senderName)}
                    senderNumber={data.senderNumber}
                    senderImg={data.senderImg}
                    time={formatToTimeAndDate(data.createdAt)} />
                </div>}
                {data.message && <span className="Message__txt">{formatTextForLinks(data.message)}</span>}
                <div className="Message__Dets">
            ...
            {(showReactions !== null) && <Modal width={'250px'} text={"Reactions"}
            closeModal={() => setShowReactions(null)} openModal={showReactions} position={"fixed"}>
                            ...
                            {filterArray(allReactions, reactionType).map((val,idx) => (
                                ...
                                style={{ cursor: val.userId == _id && "pointer" }}>
                                    ...
                                    <div className="RDM-txt">
                                        <span>
                                            {contacts.find(c => c.userId == val.userId)?.userName||val.phoneNumber}
                                        </span>
